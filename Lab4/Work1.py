...
1. 选择灰度图像，选择合适的排列方式，实现行程编码压缩,计算压缩比
2. 选择灰度图像，对图像进行8*8的分块后实现哈夫曼压缩,计算压缩比
'''
import cv2 as cv
import numpy as np
...
##彩色图像灰度化
image = cv.imread("Figure/1.jpg")
# image = cv.imread("Figure/2.jpg")
...
#压缩率
ys_rate = 100 - (len(image3) + len(data))/len(image1)*100
...
rec_image = np.hstack((grayimg, rec_image))
cv.imshow('rec_image',rec_image) #重新输出二值化图像
while cv.waitKey(100) != 27:# loop if not get ESC
    if cv.getWindowProperty('rec_image',cv.WND_PROP_VISIBLE) <= 0:
        break
# cv.destroyWindow('rec_image')
...
def put(path):
    ...
    src_img = cv.imread(path, cv.IMREAD_GRAYSCALE)
    # 记录原始图像的尺寸，后续还原图像要用到
    ...
    writeBinImage(img_encode, 'Lab4/huffman_bin_img_file.bin')

    # 读取编码的文件，得到二进制编码数据字符串
    img_read_code = readBinImage('Lab4/huffman_bin_img_file.bin', len(img_encode))
